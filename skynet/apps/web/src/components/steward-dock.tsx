...
let thread: Msg[] = [];
let draftCache = "";
// A manual 📌 pin (module-scoped so it survives navigating between pages within a
// session). When set it's a HARD focus that overrides the page you're on and the
// project Steward resolved from the conversation — so you can pin one project and
// keep multitasking on other pages without the dock re-deciding each turn.
...
export function StewardDock({
  ...
  // A manual 📌 pin — a HARD focus that wins over the page focus and Steward's
  // own resolved focus, so you can lock the dock to one project while multitasking.
  const [pinned, setPinned] = useState<string | null>(pinnedId);
  const threadRef = useRef<HTMLDivElement>(null);
  // Precedence: pin > page focus > the project Steward resolved from the chat.
  const effFocusId = pinned ?? focusProjectId ?? resolvedId;
  const effFocusName =
    projects.find((p) => p.id === pinned)?.name ?? focusProjectName ?? projects.find((p) => p.id === resolvedId)?.name ?? null;
  // Offer a one-click re-pin when the page you're on differs from the pin.
  const pageDiffers = !!pinned && !!focusProjectId && focusProjectId !== pinned;

  useEffect(() => { pinnedId = pinned; }, [pinned]); // persist the pin across pages
  ...
  const ask = async (q: string) => {
    ...
    try {
      ...
      // Pass a HARD focus only — the 📌 pin or the page you're on. With neither,
      // leave it off so Steward interprets + re-decides the project each turn.
      ...
      // header + later turns reflect the project it's now working on (unless pinned).
      if (!hardFocus && projectId) setResolvedId(projectId);
      ...
      <div className="steward-head">
        ...
        {effFocusName ? (
          <button
            ...
            title={pinned ? "Unpin — let Steward follow the page / re-decide each turn" : `Pin Steward to “${effFocusName}” so it stays here while you multitask`}
          >
            {pinned ? "📌" : "○"} {effFocusName}
          </button>
        ) : (
        ...
        {pageDiffers && (
          <button className="btn btn-ghost btn-sm" onClick={() => setPinned(focusProjectId)} title="Re-pin to the project on this page">
            ↪ pin this page
          </button>
        )}
