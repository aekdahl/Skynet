...
import { previewEnv } from "../preview/project-preview.js";
  ...
  async start(opts: FlyDeployStartOpts): Promise<FlyDeployState> {
    ...
    try {
      ...
      if (cfg.buildCmd) {
        ...
        await ensureDeps(p.dir, opts.gitRepo, (l) => this.log(p, l), previewEnv());
        this.log(p, `▸ ${cfg.buildCmd}`);
        await runToCompletion(cfg.buildCmd, p.dir, (l) => this.log(p, l), 5 * 60_000, previewEnv());
