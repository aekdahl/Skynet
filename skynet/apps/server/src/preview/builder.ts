...
function failurePage(runId: string, error: string, log: string[]): string {
...
<div class="sub">Build for <span class="id">${esc(runId)}</span> did not produce a renderable artifact.</div>
<div>${esc(error)}</div>
${tail ? `<pre>${tail}</pre>` : ""}
</body></html>`;
}
...
export class PreviewBuilder {
  ...
  private queue: Array<{ runId: string; branch: string }> = [];
  private draining = false;

  stateOf(runId: string): BuildState | undefined {
    return this.states.get(runId);
  }

  /** Lazy/idempotent: build once. Skips if in-flight or already built. */
  request(runId: string, branch: string): void {
    const s = this.states.get(runId);
    if (s) return; // queued/building/ready/failed — don't re-trigger on every view
    this.enqueue(runId, branch);
  }

  /** Force a fresh build regardless of prior state (e.g. branch advanced). */
  rebuild(runId: string, branch: string): void {
    this.enqueue(runId, branch);
  }

  private enqueue(runId: string, branch: string): void {
    this.set(runId, "queued");
    this.queue.push({ runId, branch });
    void this.drain();
  }

  private set(runId: string, status: BuildStatus, patch: Partial<BuildState> = {}): void {
    const prev = this.states.get(runId);
  ...
  private async drain(): Promise<void> {
    ...
    while (this.queue.length) {
      ...
      await this.run(job.runId, job.branch).catch(() => {});
    }
    this.draining = false;
  }

  private async run(runId: string, branch: string): Promise<void> {
    this.set(runId, "building", { log: [] });
    const log: string[] = [];
    const append = (chunk: string) => {
      for (const line of chunk.split("\n")) if (line.trim()) log.push(line);
      while (log.length > LOG_TAIL) log.shift();
      this.set(runId, "building", { log: [...log] });
    };

    let src = null as Awaited<ReturnType<typeof resolveSource>>;
    try {
      src = await resolveSource(runId, branch);
      if (!src) {
        await this.publishFailure(runId, "No preview source configured for this branch.", log);
        return;
      }
      ...
      if (previewConfig.installCmd) {
        ...
        if (r.code !== 0) return void (await this.publishFailure(runId, `install failed (exit ${r.code})`, log));
      }
      ...
      if (previewConfig.buildCmd) {
        ...
        if (r.code !== 0) return void (await this.publishFailure(runId, `build failed (exit ${r.code})`, log));
      ...
      if (!existsSync(join(publishDir, "index.html"))) {
        ...
        return void (await this.publishFailure(runId, `no index.html in ${where}`, log));
      }

      await this.publishDir(runId, publishDir);
      this.set(runId, "ready", { log });
      append(`published → ${runId}/`);
    } catch (err) {
      await this.publishFailure(runId, (err as Error).message, log);
    } finally {
      if (src) await src.cleanup().catch(() => {});
    }
  }

  // ── publish (atomic swap into <artifactRoot>/<runId>) ────────────────────
  ...
  private async swap(runId: string, staging: string): Promise<void> {
    const live = resolve(previewConfig.artifactRoot, runId);
    await mkdir(dirname(live), { recursive: true });
    await rm(live, { recursive: true, force: true });
    await rename(staging, live); // same filesystem (both under artifactRoot)
  }

  private async publishDir(runId: string, fromDir: string): Promise<void> {
    const staging = await this.stagingDir();
    await cp(fromDir, staging, { recursive: true });
    await this.swap(runId, staging);
  }

  private async publishFailure(runId: string, error: string, log: string[]): Promise<void> {
    const staging = await this.stagingDir();
    await writeFile(join(staging, "index.html"), failurePage(runId, error, log), "utf8");
    await this.swap(runId, staging);
    this.set(runId, "failed", { log, error });
  }
}
