...
export class Orchestrator {
  ...
  private async spawnRunForTask(
    ...
    opts: { baseRef?: string; bakeoffId?: string | null; beforeStart?: (run: TaskRun) => Promise<void> } = {},
    ...
    try {
      ...
      const handle = await provider.start(
        { runId, projectId: project.id, task: brief, model: runner.model, branch, cwd, apiKey, baseUrl, rates, browser: browserTools, planModeGate: project.planModeGate, disallowedTools: project.disallowedTools },
        this.events(),
      );
  ...
  private async draftClarificationAnswer(ws: string, task: Task, questions: string[]): Promise<string | null> {
    ...
    const prompt = [
      ...
      .filter((l) => l !== "")
      .join("\n");
    const reply = (await oneShotText({ prompt, model: CLARIFY_DRAFT_MODEL, apiKey, baseUrl })).trim();
    return reply || null;
  }
