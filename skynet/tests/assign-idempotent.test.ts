...
// agent, overwriting task.agentId and orphaning the first agent + its runner.
...
function seed(store: MemoryStore, taskState: Task["state"] = "backlog") {
  ...
  const task: Task = {
    id: "t1", workspaceId: DEFAULT_WORKSPACE, projectId: "p1", text: "do the thing", state: taskState, agentId: null,
  };
  // Two idle runners: proves the second assign doesn't grab the spare one.
  const runners: Runner[] = ["r1", "r2"].map((id) => ({
  ...
  return Promise.all([
    ...
    store.putTask(task),
    ...runners.map((r) => store.putRunner(r)),
  ]);
}

const busyCount = async (store: MemoryStore) =>
  (await store.listRunners(DEFAULT_WORKSPACE)).filter((r) => r.status === "busy").length;
...
describe("assignTask is idempotent (DEF-003) and refuses done tasks (DEF-005)", () => {
  it("re-assigning an assigned task returns the same agent, spawns no second agent, leaks no runner", async () => {
    ...
    expect((await store.getTask("t1"))?.agentId).toBe(first.id);
    ...
    expect((await store.getTask("t1"))?.agentId).toBe(first.id); // pointer unchanged

    // Exactly one agent exists in the workspace (no orphan B).
    expect((await store.listAgents(DEFAULT_WORKSPACE)).length).toBe(1);
  });
