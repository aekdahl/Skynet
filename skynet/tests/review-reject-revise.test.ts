// Regression guard: clicking "Reject" on a `diff` or `merge` HITL used to be a
// silent no-op once the run parked in review (no live handle — deliver() had no
// path for reject on those kinds, so it fell through to the "not delivered" log
// line and the operator saw nothing happen). Reject on diff/merge now mirrors
// the verifier gate's existing reject semantics: bounce the agent back to
// revise (reviseAfterReview), same as Modify, working even with zero typed
// guidance. Drives the REAL orchestrator against a throwaway git repo, same
// pattern as review-modify-revise.test.ts.
...
beforeAll(async () => {
  repo = mkdtempSync(join(tmpdir(), "skynet-reject-revise-repo-"));
  worktreesDir = mkdtempSync(join(tmpdir(), "skynet-reject-revise-wt-"));
