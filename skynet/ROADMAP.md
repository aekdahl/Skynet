...
- [x] **Feature-branch hierarchy — merge a Feature's tasks as a unit.** A different axis from the Feature
  *grouping* above (that's task organization; this is the branch/merge mechanics underneath it). Every task
  under a Feature now branches from — and merges into — a shared `skynet/feature/<id>` branch (lazily
  created off the project base, the same parent-branch mechanism `fork()` already used for a single parent,
  generalized to "the Feature's branch" as the shared parent for all its siblings) instead of going straight
  to the project integration branch. Once every task in the Feature reaches done, a human-gated
  `feature-merge` HITL auto-raises — reusing the same diff/checks-gate machinery a task-level merge uses,
  just retargeted to a Feature-vs-base diff — and only on approval does the Feature branch merge up into the
  project base as a whole. A task with no Feature is unaffected. Wrap, don't rebuild: `MergeEngine` gained a
  `featureId`/`targetOverride` on `MergeRequest` and a `featureBranch()` naming sibling to
  `integrationBranch()`; no changes to the Feature model, its UI, or milestone rollup.
