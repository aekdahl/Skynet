...
- [x] **Roadmap document parser and line identity (TASK 27, Phase 24 — Fleet Governance Rollout turn 4, the foundation for everything in it).** Parses `ROADMAP.md` into a stable, diffable structure — no UI, nothing else in turn 4 exists yet to consume it. New `packages/shared/src/roadmap-doc.ts` schemas (`RoadmapDoc`/`RoadmapLine`/`RoadmapAstNode`/`RoadmapSection`/`RoadmapStateBlock`) and a new `apps/server/src/roadmap/` module: `ast.ts` (the markdown parser + serializer), `identity.ts` (line-id reconciliation), `sections.ts`, `state-block.ts`, `sync.ts` (orchestration), `repo-commit.ts` (local HEAD sha). **Round-trip by construction, not reconstruction**: every AST node keeps `raw` — the exact original source span it came from — and serialization is just `ast.map(n => n.raw).join("")`. Metadata (heading level, checklist marker, extracted links, table cells) is read OFF that raw text, never used to regenerate it, so a shape the parser doesn't deeply model (a code fence, a blockquote — neither appears in ROADMAP.md today) still round-trips untouched via an "other" catch-all. Verified byte-identical against 4 real excerpts of the actual ROADMAP.md (preamble+table, mixed-checkbox-state entries, nested bullets) **plus the full current 1855-line file itself**, and a synthetic edge-case fixture (star/plus bullets, an escaped table pipe, a code fence, no trailing newline) — `tests/roadmap-ast-roundtrip.test.ts`. Correctly handles the 3-state checkbox `ROADMAP.md` actually uses (`[ ]`/`[x]`/`[~]` — confirmed live via `grep`, 36/71/15 real occurrences respectively), not just the 2-state `[ ]`/`[x]` `tasks/checklist.ts` already parses for repo-TODO-file sync (a different, narrower, pre-existing feature — left untouched). **Line identity** (`assignLineIdentity`, 3-pass): (1) exact match — a line's content hash, or an already-embedded `<!--#id-->` anchor, matches a previous parse's line, position-independent so an insertion/deletion anywhere else never touches it; (2) similarity match — a Jaccard token-overlap score (≥0.6) pairs up a genuinely reworded line with its old self among whatever's left unmatched, and ONLY this pass ever stamps a trailing anchor comment (reusing the old id) — never proactively on every line, so a project's first-ever sync adds zero anchors; (3) whatever's left is genuinely new, id = its own content hash. Verified live with real reconciliation scenarios, not just unit fixtures: `tests/roadmap-line-identity.test.ts` covers an insertion elsewhere in the file, a rewording of one line leaving unrelated lines' ids untouched, the anchor-injection mechanics, idempotent re-parsing of an already-anchored line, and a genuinely deleted line's id correctly NOT reassigned to a survivor. **State-block serializer** (`state-block.ts`): a machine-generated `<!--roadmap-state ... -->` JSON snapshot a later task can embed/diff — entries sorted by lineId and every object built with a fixed key order so regenerating unchanged data is byte-identical, always wholesale, never merged. **Webhook**: `github/webhook.ts`'s `push` event (previously a bare 202 fallthrough) now parses `{repo, commitSha, touchedPaths}` across EVERY commit in the push, not just the latest (`parseGithubPush`, pure); `Operations.handleGithubRoadmapPush` resyncs every project bound to that repo whose EFFECTIVE roadmap path (the explicit `roadmapPath` override tried exclusively when set, else the default `ROADMAP_PATHS` candidates — matching `resolveRoadmapDoc`'s own contract) the push actually touched, via the new `Operations.syncProjectRoadmap` — which writes an intermediate `repo_ahead` marker (new commitSha, before the read+parse even starts) so a slow GitHub fetch's mid-flight state is honest, then lands on `in_sync` or `unparseable`. **New store-backed cache**: `Store.getRoadmapDoc`/`putRoadmapDoc`, one `RoadmapDoc` per project — `roadmap_docs` Postgres table (`project_id` PK, same `github_connections`/`workspace_settings`/`autonomy_breakers` single-record-per-key pattern), a `Map` in memory/file stores, wired into `FileStore`'s load/flush for desktop durability. Regression-guarded end-to-end against a REAL local git repo (not mocked): `tests/roadmap-webhook-sync.test.ts` drives an actual signed HTTP push through the real Fastify route, confirms the project's cached doc lands `in_sync` with the real repo content, confirms a second sync correctly preserves an untouched line's id across a real commit, and confirms an unsigned/mismatched-path/wrong-repo push correctly syncs nothing. **A genuine, incidental fix found while wiring the store**: `FileStore`'s generic `fillArray` helper was typed `schema: z.ZodType<T>` (Input defaulting to Output) — correct for every existing flat-object schema, but `RoadmapDoc`'s nested discriminated union (via `RoadmapLine.extend()`'s `.nullable().default()` fields) has a real Input/Output divergence that surfaced a TypeScript inference error; retyped as `schema: S extends z.ZodTypeAny` returning `z.infer<S>[]`, a strictly more correct signature that changes nothing for any existing caller (verified: the full server typecheck stays clean). All 5 build steps from the task spec landed; every enrichment field the schema forward-declares (`author`/`authorRef`/`addedAt`/`taskIds`/`state`-beyond-the-checkbox/`promisedDate`/`forecast`/`questionIds`/`acceptanceCriteria`) is honestly null/[]/false today — nothing in this task's own build list asked for git-blame attribution or a linking UI, so none is fabricated; a later task populates them.
...
- [x] **Session circuit-breaker — a stuck autonomous SWEEP halts for a human, not just a stuck run.**
  ...
  *behaviorally*. Now: `config.autonomyMaxConsecutiveFailures` (`SKYNET_AUTONOMY_MAX_CONSECUTIVE_FAILURES`,
  ...
  autonomous — a manually-supervised project isn't "sweeping". Was in-memory, keyed by project id (a restart
  reset it to 0 — failed OPEN, one more attempt allowed before it could trip again); **TASK 19 (Phase 16)
  later persisted this** (`store.{get,put}AutonomyBreaker`) so a restart mid-streak no longer loses progress,
  and added a durable trip/lift audit trail plus a bounded manual override — see that entry elsewhere on this
  roadmap for the full detail; the trip/reset LOGIC below is otherwise unchanged. Re-enabling the
